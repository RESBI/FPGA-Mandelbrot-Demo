...
The default RTL is now the generic 4-context worker on XC7K70T. It builds, programs, meets timing, and passes board tests on the new part, with substantially better deep-scene throughput than the earlier 2-context default. The tradeoff is high LUT use (`88.70%`), so the historical 2-context worker remains an important lower-LUT comparison point. The earlier 4/8-context feasibility data on xc7z010 and the ring/lookahead follow-up remain documented research paths. See [CONTEXT_WORKER_ARCHITECTURE_REPORT.md](doc/CONTEXT_WORKER_ARCHITECTURE_REPORT.md) for the historical data.
...
|---|---|---|
| `build_fp64.tcl` | `SCHED_MODE=1 DYNAMIC_OWNER_DEPTH=4096 WORKER_CONTEXTS=4` | Default FP64 dynamic 4-context build. |
...
Latest default 4-context 1080p host-tiled results at 12 Mbaud with compute tile equal to host tile (`1920x120` for 1080p):
...
Measured historical dynamic + 2-context 1080p single-burst examples, all using FP64, four workers, `WORKER_CONTEXTS=2`, and 12000000 baud:
...
|---|---|---:|---:|---:|---:|---:|
| `build_fp64.tcl` on `xc7k70tfbg676-1` | Dynamic idle-core rows + tiled response | 4 | `0.583ns` | `0.000ns` | `0.039ns` | `0.000ns` |
| Historical `build_fp64.tcl` 2ctx baseline | Dynamic idle-core rows + tiled response | 2 | `1.148ns` | `0.000ns` | `0.042ns` | `0.000ns` |

Latest placed utilization for the XC7K70T default 4ctx build:
...
Historical 2ctx XC7K70T placed utilization:
...
| 12M tiled, 2ctx host tile as compute tile | Previous default, `1920x120` at 1080p | `5.127s` | `4.731s` | `19.440s` | `83.561s` |
| 12M tiled, 4ctx host tile as compute tile | Current default, `1920x120` at 1080p | `4.683s` | `5.782s` | `9.836s` | `44.146s` |

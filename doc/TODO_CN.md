...
本文对应 `TODO.md`，记录当前 VMC_RTSB ZU4EV、24.576 MHz、12-worker、8-context、6.144 Mbaud、dynamic row、tiled response 设计之后的后续工作。
...
| 默认 compute tile | 等于 host tile，宽度上限 4096 |
...
- 当前 host/RTL 默认 baudrate 为 6.144 Mbaud，host 命令发送使用 50 us byte gap。
...
- 增加 compute tile 控制；当前默认 compute tile 等于 host tile，宽度上限 4096。
...
- 将已验证的 ZU4EV `12 workers / 8 contexts` 设为默认：`WNS=25.024ns` timing clean，`160x120` verify PASS，1080p 六场景 PASS 且 0 retry。

## P0 - 可靠性和正确性

### 当前 Compute Tile 默认板级 Soak

之前 30-run 1080p 稳定性数据使用旧 host-tile retry。当前默认 compute tile 等于 host tile，并在失败后自动 soft reset；虽然已完成一次六场景 PASS，但仍需要多轮板级 soak。

任务：

- 用当前默认 host/compute tiling 重跑六场景 1080p stability benchmark。
...
6.144 Mbaud UART 当前可用，但命令 burst 需要 byte gap；更强 transport 可同时消除命令方向 margin 问题并提升 payload 方向吞吐。
...
Generic 8ctx 已在 ZU4EV 上以 12 workers 通过板级验证，但 CLB LUT 占用 `96.71%`。继续扩展前需要更低 LUT 的 worker/control 结构，不能只堆相同 worker。

任务：

- 设计低控制/寄存器开销的专用 8/12/16-context worker。
- 先复用 1 个 multiplier + 1 个 adder；context 不够前不要增加 FP units。
- 对 8ctx/12ctx/16ctx 候选重新跑 pipeline simulator 和 RTL sim。
...
### 重新调显式 compute tile size

当前默认为了降低 command overhead，使用 host tile 作为 compute tile。更小的显式 compute tile 在更看重 retry cost 时仍可能有价值。

任务：

- sweep `--compute-tile-width 256/512/1024/2048/1920`，保持 `--tile-height 120`。
- 比较 retry cost、command overhead 和六场景总时间。
- 如果 host tile = compute tile 不是最佳折中，更新默认和文档。

### 降低 Python Host Overhead

当 compute tile 变小或场景接近传输上限时，host command/parser 开销更重要。

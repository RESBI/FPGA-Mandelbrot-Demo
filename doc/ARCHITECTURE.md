...
This project implements a UART-controlled Mandelbrot accelerator on FPGA. The current board target is XC7K70T (`xc7k70tfbg676-1`) with a 200 MHz differential input clock (`CLK_200_P/N`) and an internal 100 MHz system clock generated by MMCM. The host sends one binary command describing a complete image or tile, and the FPGA streams back one 16-bit iteration count per pixel. The current default compute configuration is FP64, four Mandelbrot workers, two pixel contexts per worker, dynamic row scheduling, `FP_CE_DIV=1`, and a 12 Mbaud fractional-NCO UART. A four-context generic worker build is also validated on XC7K70T as an optional high-LUT performance experiment.
...
| Validated optional contexts per worker | 4 |
...
| Optional 4ctx routed timing | `WNS=0.583ns`, `TNS=0.000ns`, `WHS=0.039ns`, `THS=0.000ns` |
| Optional 4ctx placed utilization | `36367` LUTs, `19149` registers, `37` DSP48E1, `9.5` BRAM tiles |
...
| `mandelbrot_core_worker_kctx` | `../rtl/mandelbrot_core_worker_kctx.v` | Parameterized 4/8-context worker. The 4-context build is validated on XC7K70T but remains optional because LUT use is high. |
...
Optional 4-context XC7K70T high-iteration examples at 12 Mbaud with `1920x120` host/compute tiles:
...
| Generic 4/8-context worker experiment | Behavioral simulation passes, but the generic context-array implementation overused LUTs on the earlier xc7z010 target and is not the deployable default. See `PIPELINE_BUBBLE_ANALYSIS.md`. |

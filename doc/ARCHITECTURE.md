...
This project implements a UART-controlled Mandelbrot accelerator on FPGA. The current board target is XC7K70T (`xc7k70tfbg676-1`) with a 200 MHz differential input clock (`CLK_200_P/N`) and an internal 100 MHz system clock generated by MMCM. The host sends one binary command describing a complete image or tile, and the FPGA streams back one 16-bit iteration count per pixel. The current default compute configuration is FP64, four Mandelbrot workers, four pixel contexts per worker, dynamic row scheduling, `FP_CE_DIV=1`, and a 12 Mbaud fractional-NCO UART.
...
| Historical lower-LUT contexts per worker | 2 |
...
| Current routed timing | `WNS=0.583ns`, `TNS=0.000ns`, `WHS=0.039ns`, `THS=0.000ns` |
| Current placed utilization | `36367` LUTs, `19149` registers, `37` DSP48E1, `9.5` BRAM tiles |
| Historical 2ctx routed timing | `WNS=1.148ns`, `TNS=0.000ns`, `WHS=0.042ns`, `THS=0.000ns` |
| Historical 2ctx placed utilization | `13726` LUTs, `14559` registers, `37` DSP48E1, `9.5` BRAM tiles |
...
| `mandelbrot_core_worker_2ctx` | `../rtl/mandelbrot_core_worker_2ctx.v` | Historical lower-LUT two-context row worker. |
...
The board provides a 200 MHz differential `CLK_200_P/N` input. `top.v` uses `IBUFDS`, `MMCME2_BASE`, and `BUFG` to generate the internal 100 MHz `sys_clk`. UART, parser, FIFO, TX controller, floating-point datapath, and Mandelbrot core all run in that single internal clock domain. The `fp_ce` signal is retained as a compile-time throttle, but the current FP64 configuration sets `FP_CE_DIV=1`, so it is asserted every internal 100 MHz clock.
...
Therefore the core and FP units advance every internal system cycle, giving true 100 MHz datapath operation while preserving one post-MMCM clock domain for the compute and UART logic.
...
Current FP64 builds use normal single-cycle timing at the internal 100 MHz `sys_clk`. No `u_core` multicycle exceptions are required. Older effective-50 MHz experiments used `FP_CE_DIV=2` plus setup/hold multicycle constraints. The full XC7K70T Mandelbrot bitstream builds successfully and meets timing.

Current routed timing after XC7K70T migration:
...
### 8.5 Historical Two-Context Worker

The historical lower-LUT worker is `mandelbrot_core_worker_2ctx.v`. It keeps the same external row-worker contract but maintains two active pixel contexts internally. Each worker still has exactly one FP64 multiplier and one FP64 adder, so the optimization does not increase worker count or FP unit count. It recovers idle FP issue slots by letting one pixel make progress while the other is waiting for an FP result. The XC7K70T default now uses the generic 4-context worker, but the 2-context design remains the clearest explanation of the tagged writeback mechanism.
...
The host-tiled mode was stress-tested on the earlier board setup with the six standard 1080p scenes, five repeated runs per scene. The first pass lost the USB serial device after 23 completed frame runs; after reconnecting the serial device, the stale failed/open-port logs were rerun with `--resume`, completing the full 30-run sweep:
...
The two retry events were recovered by recomputing one 1920x120 host tile in the earlier host-tile-only implementation. The current host again uses the host tile as the default compute tile to reduce command overhead; failed attempts still drain stale UART bytes, issue `RST!RST!`, and recompute only that tile. Without host-driven tiling, an equivalent byte slip in a monolithic 4 MiB response would normally invalidate the entire frame. Exact HW/SW match is not used as the transport pass criterion for deep scenes because FP64 boundary differences are already documented separately.
...
Current default dynamic + 4-context high-iteration examples at 12 Mbaud with `1920x120` host/compute tiles:

| Case | FPGA Time | Throughput | Main limiter |
|---|---:|---:|---|
| `1080p Seahorse zoom @512` | `9.836s` | `210825.06 pps` | Mixed compute/output |
| `1080p deep tendrils @8192` | `17.677s` | `117303.25 pps` | Mostly compute |
| `1080p deep minibrot @8192` | `44.146s` | `46971.46 pps` | Compute-bound |
| `1080p deep Seahorse @1024` | `19.965s` | `103861.51 pps` | Mostly compute |

Historical 12 Mbaud single-burst 2-context examples:
...
Latest representative default dynamic + 4-context FP64 placed utilization for the current 12 Mbaud tiled-response build:
...
|---|---|---:|---:|---:|---:|---:|
| `../build_fp64.tcl` | Dynamic idle-core rows + tiled response | 4 | 0.583 ns | 0.000 ns | 0.039 ns | 0.000 ns |
...
| Four-context worker | Default on XC7K70T. It improves deep scenes but uses `88.70%` of LUTs, leaving limited headroom. |
| Generic 8-context worker experiment | Behavioral simulation passes historically, but the generic context-array implementation overused LUTs on the earlier xc7z010 target; higher-context work needs a lower-LUT shape. See `PIPELINE_BUBBLE_ANALYSIS.md`. |

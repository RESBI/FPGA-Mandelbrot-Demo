...
| 当前 timing | `WNS=0.583ns`, `TNS=0.000ns`, `WHS=0.039ns`, `THS=0.000ns` |
| 当前资源 | `36367` LUTs, `19149` registers, `37` DSP48E1, `9.5` BRAM tiles |
| 历史 2ctx timing | `WNS=1.148ns`, `TNS=0.000ns`, `WHS=0.042ns`, `THS=0.000ns` |
| 历史 2ctx 资源 | `13726` LUTs, `14559` registers, `37` DSP48E1, `9.5` BRAM tiles |
...
| `mandelbrot_core_worker_2ctx` | 历史低 LUT 2-context worker。 |
...
旧单 context worker 保留为 regression 路径。`mandelbrot_core_worker_kctx` 支持 4/8 context；4ctx 在 XC7K70T 上已通过 bitstream、烧录、小图 verify 和 1080p 六场景测试，当前是默认部署路径。其 LUT 占用达到 `88.70%`，因此 8ctx 仍需要更低 LUT 结构。
...
| WNS | TNS | WHS | THS |
|---:|---:|---:|---:|
| `1.148ns` | `0.000ns` | `0.042ns` | `0.000ns` |
...
| 2-context worker | 历史低 LUT 配置，timing margin 高，但只能隐藏部分 FP latency。 |

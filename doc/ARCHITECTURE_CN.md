...
| 当前 timing | `WNS=0.015ns`, `TNS=0.000ns`, `WHS=0.002ns`, `THS=0.000ns` |
| 当前资源 | `20288` LUTs, `17202` registers, `37` DSP48E1, `9.5` BRAM tiles |
...
默认 worker 为 `mandelbrot_core_worker_kctx`，配置为 4 context。每个 worker 有一个 FP64 multiplier 和一个 FP64 adder，并维护 4 个像素上下文。一个上下文等待 FP 结果时，其他上下文可以向 FP pipeline issue 新操作。
...
旧单 context 和 2-context worker 保留为 regression/对比路径。当前 4ctx kctx 在 XC7K70T 上已通过 bitstream、烧录、小图 verify 和 1080p 六场景测试，当前是默认部署路径。direct-200MHz 版本加入 request slicing：
...
| WNS | TNS | WHS | THS |
|---:|---:|---:|---:|
| `0.015ns` | `0.000ns` | `0.002ns` | `0.000ns` |

direct-200MHz 10 轮 1080p 稳定性测试，全部 transport pass：
...
| 4ctx generic worker | 当前默认配置，可部署并提升深场景，但 LUT 占用高。 |
| 2-context worker | 保留为低 LUT 对比/regression 配置；历史数据见演进文档。 |
| 8ctx generic worker | 尚未作为默认候选验证，预计仍需更低 LUT 结构。 |
| direct-200MHz 模式 | 已 timing-clean 且硬件验证通过，但适合作为深场景显式模式；当前 10 轮数据中 fast escape 慢于 100MHz 4ctx，standard 和深场景更快。 |
| FP64 | 深 zoom 会受精度影响。 |

...
旧单 context worker 保留为 regression 路径。实验性 `mandelbrot_core_worker_kctx` 支持 4/8 context 行为仿真，但通用数组式实现曾在旧 xc7z010 目标上 LUT 超量，当前仍不是默认部署路径。

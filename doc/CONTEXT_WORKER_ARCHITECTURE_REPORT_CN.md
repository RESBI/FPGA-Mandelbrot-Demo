...
| 面积目标 | 4/8ctx 在旧 xc7z010 上不可部署；4ctx 在 XC7K70T 上已可部署但 LUT 很高。 | 目标是让 8/12/16ctx 有部署可能，并降低 4ctx LUT。 |
...
推荐的 RTL 演进不是继续扩展 `mandelbrot_core_worker_kctx`，而是新增一个显式低 LUT worker，例如 `mandelbrot_core_worker_4ring` 或后续 `8/12ctx` slot worker。当前 XC7K70T 默认已使用 generic 4ctx，但它的 LUT 占用很高。

建议更改：

1. 保留当前 worker 外部接口，避免改 `mandelbrot_multicore`、per-core FIFO 和上层 raster merge contract。
2. 初版可从 `4ctx 1M+1A` 的低 LUT 等价结构开始，也可直接探索 `8ctx/12ctx 1M+1A`；不要同时加入第二 adder。

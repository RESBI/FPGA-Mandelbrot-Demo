...
module tb_multicore();
    ...
    mandelbrot_multicore #(.CORE_COUNT(4), .CORE_FIFO_DEPTH(128)) u_dut (
        .clk(clk),
        .rst(rst),
        .ce(ce),
        .start(start),
    ...
    initial begin
        ...
        while (pix < rows * cols) begin
            ...
            if (fifo_wr) begin
                ...
                if (fifo_data !== expected) begin
                    $display("FAIL pix=%0d y=%0d x=%0d got=%0d expected=%0d", pix, y, x, fifo_data, expected);
                    errors = errors + 1;
                end
                pix = pix + 1;
            end
        ...
        if (errors != 0) begin
            $display("=== MULTICORE TEST FAIL: %0d errors ===", errors);
            $finish;
        end

        $display("=== MULTICORE TEST PASS: %0d pixels ===", pix);
        $finish;
    end

...
这是一个基于 FPGA 的 Mandelbrot 渲染器。PC 通过 UART 发送一次图像命令，包含中心坐标、像素步长、最大迭代次数和图像尺寸；FPGA 使用 12 个 FP64 worker 计算像素，并以 16 位迭代次数流式返回。当前默认构建面向 VMC_RTSB ZU4EV，在 E12 单端 200MHz `sys_clk` 下使用 12 个 worker、每 worker 8 个像素上下文，每个 worker 共享一个 FP64 乘法器和一个 FP64 加法器；该默认构建已通过 timing、烧录、小图 HW/SW 校验和 1080p 六场景 benchmark。

详细硬件架构见 `doc/ARCHITECTURE_CN.md`，架构演进见 `doc/ARCHITECTURE_EVOLUTION_REPORT_CN.md`，worker 去气泡分析见 `doc/PIPELINE_BUBBLE_ANALYSIS.md`，N-context 新旧结构对比见 `doc/CONTEXT_WORKER_ARCHITECTURE_REPORT.md`。当前 ZU4EV 200MHz 适配、资源、时序和性能对比见 `doc/VMC_RTSB_ZU4EV_200MHZ_OPT_REPORT.md`。
...
| 历史 7K70T 参考 | `doc/200MHZ_ATTEMPT_REPORT.md`, `doc/WORKER_COUNT_SCALING.md` |
| 浮点模式 | FP64 |
| Mandelbrot worker | 12 |
| 每 worker 像素上下文 | 8 |
| 历史低 LUT 上下文 | 2, 4 |
...
| 最大迭代次数 | 65535 |
| 最大已验证帧 | 1920x1080 |
| 当前板级构建状态 | ZU4EV 完整 FP64 bitstream 已通过 |
| 烧录链路 | Vivado hardware auto-connect，目标设备 `xczu4_0` |
| 当前 routed timing | `WNS=0.148ns`, `TNS=0.000ns`, `WHS=0.010ns`, `THS=0.000ns` |
| 当前 routed utilization | `85171` LUTs, `71453` registers, `121` DSP48E2, `25.5` BRAM tiles |

当前默认 RTL 是 ZU4EV 上已验证的 direct-200MHz 12-worker、8-context-per-worker 配置。7K70T 4-worker/6-worker direct-200MHz 版本保留为历史对比点。
...
| 12-worker 8ctx 默认 | 当前默认，ZU4EV 可构建、timing-clean、可烧录并完成 1080p 六场景测试。 |

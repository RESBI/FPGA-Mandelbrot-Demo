...
module fp_mul (
    ...
    assign man_product = full_man_a * full_man_b;

    wire result_sign = sign_a ^ sign_b;
    wire [`FP_EXP_W:0] exp_sum = {1'b0, exp_a} + {1'b0, exp_b};

    // Stage 2: register DSP output
    ...
    always @(posedge clk) begin
        if (ce) begin
            ...
            result_sign_dsp_r <= result_sign;
            exp_sum_dsp_r     <= exp_sum;
            a_zero_dsp_r      <= a_is_zero;
            b_zero_dsp_r      <= b_is_zero;
        end
    end

    // Stage 3: register multiply result metadata with DSP output
